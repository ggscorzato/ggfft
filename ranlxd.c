...
*
*******************************************************************************
*   modified by M. Cristoforetti inluding gaussian generator
*******************************************************************************/
...
void testGauss(){
	FILE *tGauss;
	tGauss=fopen("testgauss.dat","w");
	int bin[1000],i,tbin;
  double rg[100000];
	double bing=10.0/1000.0;
	for(i=0;i<1000;i++) bin[i]=0;
  gauss_vectord(rg,100000);
	for(i=0; i<=100000;i++){
		tbin =(int)((rg[i]+5.0)/bing);
		if (tbin<0) tbin = 0;
		if (tbin>=1000) tbin=999;
		bin[tbin]+=1;
	}
	for (i=0;i<1000;i++){
		fprintf(tGauss,"%d\n", bin[i]);
